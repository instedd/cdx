...
describe DailyDemoData do
  ...
  before(:each) do
    ...
    device_model1 = DeviceModel.create! name: 'demo_fio_model', published_at: 1.day.ago
    manifest1 = Manifest.create! device_model: device_model1, definition: IO.read(File.join(Rails.root, 'db', 'seeds', 'manifests', 'deki_reader_manifest.json' ))
    device1 = Device.create! site: site, institution: site.institution , name: 'demo-device_fio', serial_number: '123',  device_model:  manifest1.device_model, time_zone: "UTC"

    device_model2 = DeviceModel.create! name: 'demo_cepheid_model', published_at: 1.day.ago
    manifest2 = Manifest.create! device_model: device_model2, definition: IO.read(File.join(Rails.root, 'db', 'seeds', 'manifests', 'genexpert_manifest.json' ))
