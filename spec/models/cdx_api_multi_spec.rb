...
describe "Cdx::Api Multi", elasticsearch: true do
  ...
  describe "with two different filters" do
    before(:each) do
      index test: {assays: [{result: :positive}], site_user: "jdoe"}
      index test: {assays: [{result: :negative}], site_user: "mmajor"}
    ...
    let(:responses) { multi_query([{'test.site_user' => "jdoe"}, {'test.patient_age' => "15yo.."}]) }
