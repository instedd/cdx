class LaboratorySamplesController < ApplicationController
  ...
  def sample_params
    lab_sample_params = params.require(:laboratory_sample).permit(:is_quality_control, :lab_technician, :volume, :inactivation_method, :isolate_name, notes_attributes: [:id, :description, :updated_at, :user_id, :_destroy], new_notes: [], test_qc_result_attributes: [ :id, files: [], test_qc_result_assays_attributes: [ :id, :_destroy ] ])
