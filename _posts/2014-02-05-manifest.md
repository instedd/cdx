...
The manifest may define `sample` and `patient` fields, besides the main `test` fields. Sample fields are stored in a separate `sample` model, in order to be shared among tests ran on the same sample, even if on different devices. Currently patient fields are stored along with the sample. `sample` and `patient` sections are not mandatory, and will depend on what information is being reported by the device.
...
* uuid - an automatically generated uuid to unequivocally identify the test in cdx
...
* _test_id_ - The id of the test generated by the device. If two tests are reported with the same id, the first will be updated by the second. - numeric or string.
...
* _status_ - The status of the test. - enum: ok / error / invalid / no_result / unknown
...
A CDX manifest may define `sample` and `patient` fields, besides the main `test` fields. Sample fields are stored in a separate `sample` model, in order to be shared among tests ran on the same sample, even if on different devices. Currently patient fields are stored along with the sample.
